...
install_virtualenv_darwin() {
  ...
  if [ "$(command -v pip3)" == "" ]; then
    ...
    if [[ -z $VIRTUAL_ENV ]]; then
      python3 -m pip install --require-hashes pip --upgrade --quiet --user;
    fi
  fi

  if [ "$(command -v virtualenv)" == "" ]; then
    python3 -m pip install --require-hashes virtualenv --quiet --user;
  fi
...
install_requirements() {
  ...
  python3 -m pip install --require-hashes install -r ${STARTHINKER_ROOT}/requirements.txt --quiet
  deactivate
